...
class EnemyBullet extends SpriteActor {
    constructor(x, y, velocityX, velocityY) {
        const sprite = new Sprite(assets.get('hosi'), new Rectangle(0, 0, 30, 30));
...
class Enemy extends SpriteActor {
    ...
    update(gameInfo, input) {
        ...
        this.x -= this._velocityX;
        this.y += this._velocityY;
        if(this.x <= 100 || this.x >= 350) {		//敵が動く範囲？
          this._velocityX *= -1;
          this._velocityY *= -1;
        }
        ...
        if(this._timeCount > this._interval) {
             if(currentHp>300){
                  this.shootCircularBullets(10, 4);		//引数１は弾幕の密度、引数２は弾速
             }else{
                  this.shootCircularBullets(10, 4);
                  this.shootCircularBullets(15, 5);
             }
...
class Enemy2 extends SpriteActor {
    constructor(x, y) {
        ...
        this.maxHp2 = maxHp;		//敵の最大HP
        this.currentHp2 = this.maxHp;
        ...
        this.addEventListener('hit', (e) => {
           if(e.target.hasTag('playerBullet')) {
               currentHp--;
               this.dispatchEvent('changehp2', new GameEvent(this));
           }
        });
    }
    ...
    update(gameInfo, input) {
        ...
        if(this._timeCount > this._interval) {
             if(currentHp>200){
                  ...
                  this.shootCircularBullets(20, 4);
                  this.shootCircularBullets(30, 3);
...
class DanmakuStgMainScene extends Scene {
    constructor(renderingTarget) {
        ...
        const enemy = new Enemy(350, 100);
        const enemy2 = new Enemy2(150, 140);
