...
function updateTimetText(){
        ...
        //rink2.textContent = '<a href=../sonota/demo.html>デモプレイに</a>';
...
class EnemyBullet extends SpriteActor {
    constructor(x, y, velocityX, velocityY) {
        const sprite = new Sprite(assets.get('kaminari'), new Rectangle(0, 0, 16, 40));
        const hitArea = new Rectangle(2, 4, 12, 32);
...
class FireworksBullet extends EnemyBullet2 {
...
class Enemy extends SpriteActor {
    constructor(x, y) {
        const sprite = new Sprite(assets.get('zeusu'), new Rectangle(0, 0, 64, 64));
        const hitArea = new Rectangle(0, 0, 64,64);
        super(x, y, sprite, hitArea, ['enemy']);

        this.maxHp = 280;		//敵の最大HP
        this.currentHp = this.maxHp;

        this._interval = 10;		//弾幕の発射間隔(初期値は30)
    ...
    update(gameInfo, input) {
       ...
       // this.x += this._velocityX;
        ...
        if(flag == 0 || this.currentHp >= 150){
             ...
             this._timeCount++;
             if(this._timeCount > this._interval) {
                  this.shootCircularBullets(22,6);
                  this.shootCircularBullets2(13, 5);		//引数１は弾幕の密度、引数２は弾速
                  this._timeCount = 0;
             }
        }
...
class Enemy2 extends SpriteActor {
    constructor(x, y) {
        const sprite = new Sprite(assets.get('tensi1'), new Rectangle(0, 0, 64, 71));
        const hitArea = new Rectangle(0, 0, 64, 71);
        super(x, y, sprite, hitArea, ['enemy']);

        this.maxHp2 = 100;		//敵の最大HP
        this.currentHp2 = this.maxHp2;

        this._intervalEX = 25;		//破裂する弾幕の発射間隔(初期値は30)
    ...
    update(gameInfo, input) {
        ...
        if(this._timeCount > this._intervalEX) {
            const spdX = Math.random() * 8 - 2; // -2〜+2
            const spdY = Math.random() * 4 +1;
            const explosionTime = 60;
            const bulletEX = new FireworksBullet(this.x, this.y, spdX, spdY, explosionTime);
...
class DanmakuStgMainScene extends Scene {
    constructor(renderingTarget) {
         ...
         const enemy = new Enemy(220, 80);
         const enemy2 = new Enemy2(220, 200);
