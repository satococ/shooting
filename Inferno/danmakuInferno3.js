...
class Fighter extends SpriteActor {
    constructor(x, y) {
        var sprite = new Sprite(assets.get('my'), new Rectangle(0, 0, 32, 32));
    ...
    update(gameInfo, input) {
        ...
        if(hanten == 0){
            if(input.getKey('Shift')){
             ...
             this.sprite = new Sprite(assets.get('my'), new Rectangle(0, 0, 32, 32));
        }else{
             if(input.getKey('Shift')){
              	if(input.getKey('ArrowRight')) { this._velocityX = -this._speedS; }
              }else{
              ...
              this.sprite = new Sprite(assets.get('alt_my'), new Rectangle(0, 0, 32, 32));
        }
        ...
        if(hanten == 1 && flga == 0){
             if(this.x <= 250){
               ...
               this.x = this.new_x;
                 ...
                 this.old_x = this.x;
                 _spritx = this.x;
                 for(let i2=250;i2<=this.old_x;i2++) {
 				_spritx = 470 - this.x -i2 ;
 			  }
 			this.x = this.x + _spritx;

             }
             flg = 1;
             flga = 1;
        }else if(hanten == 0 && flg == 1){
             if(this.x <= 250){
              this.old_x = this.x;
...
class Enemy extends SpriteActor {
    constructor(x, y) {
        const sprite = new Sprite(assets.get('teki'), new Rectangle(0, 0, 64, 64));
        ...
        this._interval = 80;		//弾幕の発射間隔(初期値は30)

        this._flag_interval = 200;      //反転用
        this._flag_timeCount = 0;		//反転用
    ...
    update(gameInfo, input) {
        ...
        this._flag_timeCount++;
        if(this._flag_timeCount > this._flag_interval) {
...
assets.addImage('alt_my', '../image/hantenhand.png');
assets.addImage('mark', '../image/マーカー.png');
assets.addImage('bom', '../image/bomb2.png');
assets.addImage('haikei', '../image/Inferno/I1.png');
assets.addImage('danmaku', '../image/Inferno/knife.png');
assets.addImage('teki', '../image/Inferno/魔法少女.png');
