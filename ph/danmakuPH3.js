...
class Enemy extends SpriteActor {
    constructor(x, y) {
        const sprite = new Sprite(assets.get('nyaru'), new Rectangle(0, 0, 358, 400));
    ...
    update(gameInfo, input) {
     ...
     if(elapsedTime<=22000){
          ...
          if(this._timeCount > this._interval) {
            //  this.shootCircularBullets(8, 2);
               this.shootCircularBullets(15, 4);		//引数１は弾幕の密度、引数２は弾速
               this._timeCount = 0;
          }
     }
        ...
        //15秒後に魔法陣を召喚する
        if(elapsedTime>=15000 && flg==0){
             const syoukan = new Mahouzin(80,60);      //魔法陣の出現場所
             this.spawnActor(syoukan);
             flg ++;
        }
        //30秒経過後クリアになる
...
class Mahouzin extends SpriteActor {
    constructor(x, y) {
        const sprite = new Sprite(assets.get('zin'), new Rectangle(0, 0, 50, 50));
        const hitArea = new Rectangle(0, 0, 50, 50);
        super(x, y, sprite, hitArea, ['zinA']);
    ...
    shootLaser(degree, speed) {
       ...
       const laser1 = new Enemylaser(this.x, this.y, velocityX, velocityY);
    ...
    update(gameInfo, input) {
        ...
        this._timeCount++;
        if(this._timeCount > this._interval) {
          this. shootCircularLaser(3, 15);    	//レーザーを発射する。引数１は弾幕の密度、引数２は弾速
           this._timeCount = 0;
        }
...
assets.addImage('nyaru', '../image/ニャルラトテップ.png');
assets.addImage('haikei', '../image/PH3.png');
assets.addImage('zin', '../image/魔法陣.png');
