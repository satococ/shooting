...
constructor(x, y) {
       const sprite = new Sprite(assets.get('tenkai'), new Rectangle(0, 0, 600, 1200));
       const hitArea = new Rectangle(0, 0, 0, 0);
...
class Enemy extends SpriteActor {
    constructor(x, y) {
        const sprite = new Sprite(assets.get('tenshi'), new Rectangle(0, 0, 64, 85));
    ...
    shootBarrier(degree, speed) {
        const rad = degree / 180 * Math.PI;		//初期値は180
        const velocityX = Math.cos(rad) * speed;
        const velocityY = Math.sin(rad) * speed;


        const barrier = new EnemyBarrier(this.x, this.y, velocityX, velocityY);
    ...
    shootCircularBarrier(num, speed) {
        const degree = 360 / num;		//初期値は360
        for(let i = 0; i < num; i++) {
            this.shootBarrier(degree * i, speed);
        }
    }
